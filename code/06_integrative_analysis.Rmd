...
sig_species = read.xlsx("../results/outputs/res_ancombc2.xlsx", sheet = 1) %>%
  filter(p_val < 0.05) %>%
  .$taxon
sig_genera = read.xlsx("../results/outputs/res_ancombc2.xlsx", sheet = 2) %>%
  filter(p_val < 0.05) %>%
  .$taxon
...
df_da_genera = data.frame(df_da_genera[-1, , drop = FALSE], check.names = FALSE) %>%
  rownames_to_column("sampleid") %>%
  mutate(across(-1, as.numeric))
df_da_genera[is.na(df_da_genera)] = 0
...
df_add_genus = data.frame(df_add_genus)

sig_species = make.names(sig_species)
sig_genera = make.names(sig_genera)

wb = createWorkbook()
```

# Microbiome vs. cytokines {.tabset}

## Cytokines vs. DA species
...
## Cytokines vs. DA genera

```{r}
t_formula = as.formula(paste0("cbind(crp, cd14, cd163) ~ ",
                              paste0(sig_genera, collapse = " + ")))
fit = manova(t_formula, data = df_add_genus)
...
addWorksheet(wb, "cyto_genera")
writeData(wb, "cyto_genera", df_summ)
```

# Microbiome vs. SCFAs {.tabset}
...
addWorksheet(wb, "scfa_genera")
writeData(wb, "scfa_genera", df_summ)
```
