...
summary_template =
  list("Primary Group" =
       ...
       "Status" =
         list("NC" = ~ n_perc0(status == "nc", na_rm = T),
              "SC" = ~ n_perc0(status == "sc", na_rm = T),
...
tidy_model = broom::tidy(model) %>%
  ...
  mutate(index = seq_len(length(coef(model)) - 1),
         label = c("Age", "Educa (Undergrad)", "Educa (Postgrad)", 
                   ...
                   "Drink (Heavy)", "Drug Use (Yes)",  "HBV (Resolved)", "HBV (Positive)"),
...
p_summ_prim = df_fig %>% 
  ...
  scale_fill_aaas(name = NULL, labels = c("NC", "SC"))
```
