...
2. Number of taxa `r ntaxa(genus_data)`.

```{r}
key_genera = c("Akkermansia", "Alistipes", "Bacteroides", "Butyricimonas", 
               "Dehalobacterium", "Methanobrevibacter", "Odoribacter")
```
...
genera1 = rownames(res_linear1$corr_fl)
genera1 = genera1[grepl("Genus:", genera1)]
genera2 = rownames(res_linear2$corr_fl)
genera2 = genera2[grepl("Genus:", genera2)]
genera3 = rownames(res_linear3$corr_fl)
genera3 = genera3[grepl("Genus:", genera3)]
genera4 = rownames(res_linear4$corr_fl)
genera4 = genera4[grepl("Genus:", genera4)]
common_genera = Reduce(intersect, list(genera1, genera2, genera3, genera4))
common_genera = sapply(common_genera, function(x) strsplit(x, ":")[[1]][2])
```

### Visualization

```{r}
df_corr1 = data_preprocess(res_linear1, type = "linear", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G1")
df_corr2 = data_preprocess(res_linear2, type = "linear", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G2")
df_corr3 = data_preprocess(res_linear3, type = "linear", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G3")
df_corr4 = data_preprocess(res_linear4, type = "linear", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G4")
...
genera1 = rownames(res_dist1$dcorr_fl)
genera1 = genera1[grepl("Genus:", genera1)]
genera2 = rownames(res_dist2$dcorr_fl)
genera2 = genera2[grepl("Genus:", genera2)]
genera3 = rownames(res_dist3$dcorr_fl)
genera3 = genera3[grepl("Genus:", genera3)]
genera4 = rownames(res_dist4$dcorr_fl)
genera4 = genera4[grepl("Genus:", genera4)]
common_genera = Reduce(intersect, list(genera1, genera2, genera3, genera4))
common_genera = sapply(common_genera, function(x) strsplit(x, ":")[[1]][2])
```
...
```{r}
df_corr1 = data_preprocess(res_dist1, type = "dist", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G1")
df_corr2 = data_preprocess(res_dist2, type = "dist", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G2")
df_corr3 = data_preprocess(res_dist3, type = "dist", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G3")
df_corr4 = data_preprocess(res_dist4, type = "dist", level = "Genus", tax = key_genera) %>%
  dplyr::mutate(group = "G4")
...
ggsave(file = "../results/figures/supp_secom.jpeg", 
       plot = fig_species_linear, height = 6, width = 10, units = "in", dpi = 100)
```
