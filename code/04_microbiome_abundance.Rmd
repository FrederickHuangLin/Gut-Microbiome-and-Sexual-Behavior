...
output = ancombc2(data = tse, assay_name = "counts", tax_level = "Species",
                  ...
                  trend_control = list(contrast = list(matrix(c(1, 0, 0,
                                       ...
                                       solver = "ECOS",
                                       B = 1000))
saveRDS(output, "../results/outputs/primary_ancombc2_species.rds")
```

## Bias-corrected abundances

```{r}
output = readRDS("../results/outputs/primary_ancombc2_species.rds")
...
df_fig = res_trend %>%
  ...
  arrange(desc(color))
df_fig$group = recode(df_fig$group, `fc2` = "G2 - G1", 
                      `fc3` = "G3 - G1", `fc4` = "G4 - G1")
df_fig$taxon = factor(df_fig$taxon, levels = unique(df_fig$taxon))
...
cell_max = max(abs(df_fig$value))
fig_group_species = df_fig %>%
...
# Primary group at genus level {.tabset}

1. Number of stool samples `r phyloseq::nsamples(genus_data)`,

2. Number of taxa `r phyloseq::ntaxa(genus_data)`.
...
output = ancombc2(data = tse, assay_name = "counts", tax_level = "Genus",
                  ...
                  trend_control = list(contrast = list(matrix(c(1, 0, 0,
                                       ...
                                       solver = "ECOS",
                                       B = 1000))
saveRDS(output, "../results/outputs/primary_ancombc2_genus.rds")
```

## Bias-corrected abundances

```{r}
output = readRDS("../results/outputs/primary_ancombc2_genus.rds")
bias_correct_log_table = output$bias_correct_log_table
bias_correct_log_table = bias_correct_log_table %>%
  rownames_to_column("genus")
write_csv(bias_correct_log_table, "../results/outputs/bias_corr_genus.csv")
```
...
res_trend = output$res_trend %>%
  ...
  mutate(taxon = strsplit(taxon, ":")[[1]][2]) %>%
  ungroup() %>%
  dplyr::filter(p_val < 0.05) %>%
  arrange(taxon)

addWorksheet(wb, "group_genus")
writeData(wb, "group_genus", res_trend)
...
# Seroconversion status at species level {.tabset}
...
saveRDS(output, "../results/outputs/status_ancombc2_species.rds")
```

## Visualization

```{r, fig.width=10}
output = readRDS("../results/outputs/status_ancombc2_species.rds")
res_prim = output$res %>%
  rowwise() %>%
  filter(grepl("Species:", taxon)|grepl("Genus:", taxon)) %>%
  mutate(taxon = ifelse(grepl("Genus:", taxon), 
...
# Seroconversion status at genus level {.tabset}
...
output = ancombc2(data = tse, assay_name = "counts", tax_level = "Genus",
                  ...
                  group = "status", struc_zero = TRUE, neg_lb = TRUE,
                  alpha = 0.05, n_cl = 4, verbose = TRUE)
saveRDS(output, "../results/outputs/status_ancombc2_genus.rds")
```

## Visualization

```{r, fig.width=10}
output = readRDS("../results/outputs/status_ancombc2_genus.rds")
res_prim = output$res %>%
  rowwise() %>%
  filter(grepl("Genus:", taxon)) %>%
  mutate(taxon = strsplit(taxon, ":")[[1]][2]) %>%
...
addWorksheet(wb, "status_genus")
writeData(wb, "status_genus", res_prim)
